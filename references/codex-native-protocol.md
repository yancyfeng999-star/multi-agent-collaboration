...
原生线程消息仍由平台负责唤醒，但 Skill 输出使用同一组精简摘要合同。工作 Agent 可以通过现有
self-service dispatch 发布子任务或抢占任务/线程；发布和候选评估只产生结构化摘要，不新增长期
角色，不改变 Protocol v3 状态机，也不要求 Coordinator 为每个独立任务转发进度。
